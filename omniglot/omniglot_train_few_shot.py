...
loss_txt = './logs/loss.txt'
test_accuracy_txt ='./logs/test_accuracy.txt'
train_accuracy_txt ='./logs/train_accuracy.txt'
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        # calculate features
        sample_features = feature_encoder(Variable(samples).cuda(GPU)) # 5x64*5*5 (5*5 => spatial size)
        ...
        total_rewards_train += np.sum(rewards)
        ...
        if (episode+1)%100 == 0:
            ...
            train_accuracy = total_rewards_train/1.0/CLASS_NUM/SAMPLE_NUM_PER_CLASS/100
            print(f"train accuracy: {train_accuracy}")
            with open(loss_txt, 'a') as f:
                f.write(str(loss.data.item()) + '\n')
            with open(train_accuracy_txt, 'a') as f:
        ...
        if (episode+1)%5000 == 0:
            ...
            with open(test_accuracy_txt, 'a') as f:
